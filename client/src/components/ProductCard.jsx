...
const ProductCard = ({ product }) => {
  ...
  // If the product is out of stock, show a placeholder
 ...
 if (!product.inStock) {
  ...
  // In-stock product: show full product card
  return (
    ...
    >
      ...
      <div className="space-y-2">
        ...
        <h3 className="text-gray-900 font-semibold text-base md:text-lg leading-tight line-clamp-2 min-h-[3rem]">{product.name}</h3>
        ...
        <div className="flex items-end justify-between pt-2">
          <div className="flex flex-col">
            <div className="flex items-center gap-2">
              <span className="text-lg md:text-xl font-bold text-primary">{currency}{product.offerPrice}</span>
              <span className="text-sm text-gray-500 line-through">{currency}{product.price}</span>
            </div>
            <span className="text-xs text-green-600 font-medium">Save ${(product.price - product.offerPrice).toFixed(2)}</span>
          </div>
